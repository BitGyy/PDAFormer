...
from pdaformer.utilities.random_stuff import no_op
from pdaformer.utilities.to_torch import to_cuda, maybe_to_torch
...
import os, json, datetime, numbers
...
def _to_jsonable(obj):
    ...
    # 其余（int/float/bool/str/None）
    return obj

def _append_metrics_log(metrics_log_path: str, case_name: str, metrics: dict):
    try:
    ...
    with open(metrics_log_path, "a", encoding="utf-8") as f:
        f.write(f"=== {stamp} | case: {case_name or 'unknown'} ===\n")
        for k in sorted(metrics.keys()):
            f.write(f"{k}: {metrics[k]}\n")
        # 关键：先转为可JSON的原生类型
        metrics_jsonable = _to_jsonable(metrics)
        f.write("metrics_json: " + json.dumps(metrics_jsonable, ensure_ascii=False) + "\n\n")

def _iter_metrics_from_log(metrics_log_path: str):
    if not (metrics_log_path and os.path.exists(metrics_log_path)):
        return
    with open(metrics_log_path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line.startswith("metrics_json: "):
                payload = line[len("metrics_json: "):]
                try:
...
# ==== 新增：计算均值（仅对数值型字段）====
def _compute_mean_metrics_dict(dicts):
    sums, counts = {}, {}
    for d in dicts:
        for k, v in d.items():
            # 先把 numpy 标量转成 python 标量
            if isinstance(v, np.generic):
                v = v.item()
            if isinstance(v, numbers.Number) and not isinstance(v, bool):
                sums[k] = sums.get(k, 0.0) + float(v)
                counts[k] = counts.get(k, 0) + 1
    mean = {k: (s / counts[k]) for k, s in sums.items() if counts.get(k, 0) > 0}
    mean["num_cases"] = int(max(counts.values()) if counts else 0)
    return mean


# ==== 新增：写入单个 case + 累计 mean ====
def _append_metrics_log_with_mean(metrics_log_path: str, case_name: str, metrics: dict):
    # 读取历史
    prev = list(_iter_metrics_from_log(metrics_log_path))
    combined = prev + [metrics]
    mean_metrics = _compute_mean_metrics_dict(combined)
    ...
    with open(metrics_log_path, "a", encoding="utf-8") as f:
        ...
        for k in sorted(metrics.keys()):
            f.write(f"{k}: {metrics[k]}\n")
        f.write("metrics_json: " + json.dumps(_to_jsonable(metrics), ensure_ascii=False) + "\n")
        # 写累计均值
        f.write(f"--- mean over {mean_metrics.get('num_cases', len(combined))} cases ---\n")
        for k in sorted(mean_metrics.keys()):
            f.write(f"{k}: {mean_metrics[k]}\n")
        f.write("mean_metrics_json: " + json.dumps(_to_jsonable(mean_metrics), ensure_ascii=False) + "\n\n")
...
class _CudaTimer:
    ...
    def __exit__(self, exc_type, exc, tb):
        self.ev_end.record()
        self.ev_end.synchronize()  # 等待结束，保证测到准确时间
    def ms(self):
        return self.ev_start.elapsed_time(self.ev_end)  # 返回毫秒

def _count_unique_voxels_after_slicing(shape_after_slice):
    # shape_after_slice = aggregated_results[slicer].shape
    # aggregated_results 的形状是 [C, X, Y, Z]，unique voxels 用 [X*Y*Z]
    return int(np.prod(shape_after_slice[1:]))

def _count_processed_voxels_per_tiles(patch_size, num_tiles):
    return int(np.prod(patch_size) * num_tiles)
...
class SegmentationNetwork(NeuralNetwork):
    def __init__(self):
        ...
        self.num_classes = 4  # number of channels in the output
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                        ...
                                        verbose: bool, profile: bool=False) -> Tuple[np.ndarray, np.ndarray, dict]:
        assert len(x.shape) == 4, "x must be (c, x, y, z)"
        assert self.get_device() != "cpu"
        if verbose: print("step_size:", step_size, "do mirror:", do_mirroring)
        assert patch_size is not None, "patch_size cannot be None for tiled prediction"

        print('using predict_3D_3Dconv_tiled')
        # === 1) 统计初始化 ===
        metrics = {}
        wall_t0 = time.perf_counter()
        if torch.cuda.is_available():
            torch.cuda.reset_peak_memory_stats(self.get_device())
        tile_times_ms = []
        move2gpu_times_ms = []
        gaussian_times_ms = []

        # 预热（可选，减少首次 kernel 编译抖动；也可提前在外层做）
        ...
        if verbose:
            print("data shape:", data_shape, "patch size:", patch_size,
                "steps (x,y,z):", steps, "number of tiles:", num_tiles)

        # === 4) 高斯权重（可计时）===
        gaussian_importance_map = None
        if use_gaussian and num_tiles > 1:
            if profile:
                gt = _CudaTimer(self.get_device()); gt.__enter__()
        ...
        if all_in_gpu:
            ...
            if profile:
                t_mv = _CudaTimer(self.get_device()); t_mv.__enter__()
            data = torch.from_numpy(data).cuda(self.get_device(), non_blocking=True)
            if profile:
                t_mv.__exit__(None, None, None); move2gpu_times_ms.append(t_mv.ms())

            if verbose: print("initializing result_numsamples (on GPU)")
            aggregated_nb_of_predictions = torch.zeros([self.num_classes] + list(data.shape[1:]), dtype=torch.half,
                                                    device=self.get_device())
        else:
            add_for_nb_of_preds = self._gaussian_3d if (use_gaussian and num_tiles > 1) else np.ones(data.shape[1:], dtype=np.float32)
        ...
        with torch.inference_mode():
            for x0 in steps[0]:
                lb_x, ub_x = x0, x0 + patch_size[0]
                for y0 in steps[1]:
                    lb_y, ub_y = y0, y0 + patch_size[1]
                    for z0 in steps[2]:
                        lb_z, ub_z = z0, z0 + patch_size[2]

                        # 只计“模型+后处理”的 GPU 时间（不含 numpy/复制）
                        t_tile = _CudaTimer(self.get_device()) if profile else None
                        if profile: t_tile.__enter__()
                        ...
                        if profile:
                            t_tile.__exit__(None, None, None)
                            tile_times_ms.append(t_tile.ms())

                        aggregated_results[:, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z] += predicted_patch
                        aggregated_nb_of_predictions[:, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z] += \
                            (add_for_nb_of_preds if all_in_gpu else add_for_nb_of_preds)

        # === 7) 反 padding、概率 & 分割 ===
        slicer2 = tuple([slice(0, aggregated_results.shape[i]) for i in
                        range(len(aggregated_results.shape) - (len(slicer) - 1))] + slicer[1:])
        aggregated_results = aggregated_results[slicer2]
        aggregated_nb_of_predictions = aggregated_nb_of_predictions[slicer2]
        ...
        if torch.cuda.is_available():
            max_mem_alloc = torch.cuda.max_memory_allocated(self.get_device()) / (1024**2)   # MB
            max_mem_resv  = torch.cuda.max_memory_reserved(self.get_device())  / (1024**2)   # MB
        else:
            max_mem_alloc = max_mem_resv = 0.0

        # 吞吐量（两种口径）：唯一体素/秒 & 处理体素/秒（包含重叠冗余）
        unique_vox = _count_unique_voxels_after_slicing(aggregated_results.shape if all_in_gpu else class_probabilities.shape)
        processed_vox = _count_processed_voxels_per_tiles(patch_size, num_tiles)

        if profile:
            tile_ms = np.array(tile_times_ms) if len(tile_times_ms) else np.array([0.0])
            metrics.update({
                "num_tiles": int(num_tiles),
                "patch_size": tuple(patch_size),
                "volume_shape_after_pad": tuple(data_shape[1:]),
                "wall_time_total_s": float(wall_total_s),
                "tile_time_ms_mean": float(tile_ms.mean()),
                "tile_time_ms_p50": float(np.percentile(tile_ms, 50)),
                "tile_time_ms_p90": float(np.percentile(tile_ms, 90)),
                "move2gpu_time_ms": float(np.sum(move2gpu_times_ms) if move2gpu_times_ms else 0.0),
                "gaussian_time_ms": float(np.sum(gaussian_times_ms) if gaussian_times_ms else 0.0),
                "throughput_unique_voxels_per_s": float(unique_vox / max(wall_total_s, 1e-6)),
                "throughput_processed_voxels_per_s": float(processed_vox / max(wall_total_s, 1e-6)),
                "overlap_efficiency": float(unique_vox / max(processed_vox, 1)),  # 越接近1越高
                "gpu_max_memory_allocated_MB": float(max_mem_alloc),
                "gpu_max_memory_reserved_MB": float(max_mem_resv),
        ...
        # _append_metrics_log_with_mean('/media/abc/disk11/PDAFormer/infer_metrics_pda.txt', "", metrics)
        # _append_metrics_log_with_mean('/media/abc/disk11/PDAFormer/infer_metrics_emnet.txt', "", metrics)
        # _append_metrics_log_with_mean('/media/abc/disk11/PDAFormer/infer_metrics_umamba.txt', "", metrics)
        # _append_metrics_log_with_mean('/media/abc/disk11/PDAFormer/infer_metrics_TCConet.txt', "", metrics)
        # _append_metrics_log_with_mean('/media/abc/disk11/PDAFormer/infer_metrics_MSTCnet.txt', "", metrics)
        # _append_metrics_log_with_mean('/media/abc/disk11/PDAFormer/infer_metrics_SwinUNETR.txt', "", metrics)

        return predicted_segmentation, class_probabilities, metrics
