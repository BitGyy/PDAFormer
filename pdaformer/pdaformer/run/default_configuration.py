...
import pdaformer
from pdaformer.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
from batchgenerators.utilities.file_and_folder_operations import *
from pdaformer.experiment_planning.summarize_plans import summarize_plans
...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
                              search_in=(pdaformer.__path__[0], "training", "network_training"),
    ...
    if task == 'Task001_ACDC':
        ...
        pickle.dump(plans, pickle_file)
        pickle_file.close()
    elif task == 'Task501_CTA':
        plans['plans_per_stage'][Stage]['batch_size'] = 1
        # plans['plans_per_stage'][Stage]['patch_size'] = np.array([224, 224, 224])
        # plans['plans_per_stage'][Stage]['patch_size'] = np.array([256, 256])
